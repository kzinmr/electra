...
class ModelRunner(object):
    ...
    def __init__(
        ...
        if self._config.do_train:
            (
                ...
                self.train_steps,
            ) = self._preprocessor.prepare_train()
        else:
    ...
    def write_classification_outputs(self, tasks, trial, split):
        ...
        results = self._estimator.predict(
            input_fn=predict_input_fn, yield_single_examples=True
        )
        ...
        for r in results:
            if r["task_id"] != len(self._tasks):
                ...
                logits[task_name][r[task_name]["eid"]] = (
                    r[task_name]["logits"]
                    if "logits" in r[task_name]
                    else r[task_name]["predictions"]
                )
        for task_name in logits:
            utils.log(
                "Pickling predictions for {:} {:} examples ({:})".format(
        ...
        for trial_results in results:
            for task_name, task_results in trial_results.items():
                ...
                results_str += (
                    ...
                    + ": "
                    + " - ".join(
                        ["{:}: {:.2f}".format(k, v) for k, v in task_results.items()]
                    )
                    + "\n"
                )
        f.write(results_str)
                    ...
                    if task.name in [
                        ...
                        for q, _ in preds.items():
                            if null_odds[q] > config.qa_na_threshold:
                                preds[q] = ""
                        utils.write_json(
                            preds, config.test_predictions(task.name, "test", trial)
                        )
                    else:
                        utils.log(
                            "Skipping task",
...
def main():
    ...
    run_finetuning(
        configure_finetuning.FinetuningConfig(args.model_name, args.data_dir, **hparams)
    )
