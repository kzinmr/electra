...
from pretrain import pretrain_data, pretrain_helpers
...
class PretrainingModel(object):
    ...
    def __init__(
        ...
        masked_inputs = pretrain_helpers.mask(config, unmasked_inputs, config.mask_prob)
        ...
        if config.uniform_generator:
            ...
            config.electra_objective or config.electric_objective
        ...
        if config.electra_objective or config.electric_objective:
            ...
            disc_output = self._get_discriminator_output(
                fake_data.inputs, discriminator, fake_data.is_fake_tokens, cloze_output
            )
    ...
    def _get_cloze_outputs(self, inputs: pretrain_data.Inputs, model):
        ...
        weights = tf.cast(
            pretrain_helpers.get_candidates_mask(self._config, inputs), tf.float32
        )
        with tf.variable_scope("cloze_predictions"):
            ...
            return get_softmax_output(
                logits, inputs.input_ids, weights, self._bert_config.vocab_size
            )
...
def get_softmax_output(logits, targets, weights, vocab_size):
    ...
    SoftmaxOutput = collections.namedtuple(
        "SoftmaxOutput",
        ["logits", "probs", "loss", "per_example_loss", "preds", "weights"],
    )
...
class TwoTowerClozeTransformer(object):
    ...
    def __init__(
        ...
        self._sequence_output = tf.concat([roll(ltr_reprs, -1), roll(rtl_reprs, 1)], -1)
...
def main():
    ...
    parser.add_argument(
        "--data-dir", required=True, help="Location of data files (model weights, etc)."
    )
    parser.add_argument(
        "--model-name", required=True, help="The name of the model being fine-tuned."
    )
    parser.add_argument(
