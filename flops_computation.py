        ...
        return (
            ...
            * (
                ...
                + (
                    ...
                    if discriminator
                    else self.get_embedding_flops(output=True)
                )
            )
        )
...
def get_electra_train_flops(
    ...
    disc = TransformerHparams(
        h_d, l_d, s=s, e=e, output_frac=output_frac
    ).get_train_flops(batch_size, train_steps, True)
    gen = TransformerHparams(
        h_g, l_g, s=s, e=e if tied_embeddings else None, output_frac=output_frac
    ).get_train_flops(batch_size, train_steps)
    return disc + gen
...
MODEL_FLOPS = collections.OrderedDict(
    [
        ...
        (
            "gpt",
            TransformerHparams(768, 12, v=40000, output_frac=1.0).get_train_flops(
                128, 960800
            ),
        ),
        (
            "bert_small",
            TransformerHparams(256, 12, e=128, s=128).get_train_flops(128, 1.45e6),
        ),
        ("bert_base", TransformerHparams(768, 12).get_train_flops(256, 1e6)),
        ("bert_large", TransformerHparams(1024, 24).get_train_flops(256, 1e6)),
        (
            "electra_small",
            get_electra_train_flops(256, 12, 64, 12, 128, 1e6, True, s=128, e=128),
        ),
        ("electra_base", get_electra_train_flops(768, 12, 256, 12, 256, 766000, True)),
        (
            "electra_400k",
            get_electra_train_flops(1024, 24, 256, 24, 2048, 400000, True),
        ),
        (
            "electra_1.75M",
            get_electra_train_flops(1024, 24, 256, 24, 2048, 1750000, True),
        ),
        ...
        (
            "roberta",
            TransformerHparams(1024, 24, v=50265).get_train_flops(8000, 500000),
        ),
        (
            "albert",
            TransformerHparams(4096, 12, v=30000, e=128).get_train_flops(4096, 1.5e6),
        ),
        (
            "t5_11b",
            TransformerHparams(
                ...
                heads=128,
                head_size=128,  # heads/head size
                output_frac=0.0,  # encoder has no output softmax
            ).get_train_flops(2048, 1e6)
            + TransformerHparams(  # 1M steps with batch size 2048
                1024,
                24,
                v=32000,
                i=65536,
