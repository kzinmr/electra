...
class Preprocessor(object):
    ...
    def __init__(self, config: configure_finetuning.FinetuningConfig, tasks):
        ...
        self._name_to_feature_config = {
            spec.name: spec.get_parsing_spec() for spec in self._feature_specs
        }
    ...
    def _serialize_dataset(self, tasks, is_training, split):
        ...
        batch_size = (
            ...
            if is_training
            else self._config.eval_batch_size
        )

        utils.log("Loading dataset", dataset_name)
        n_examples = None
        if self._config.use_tfrecords_if_existing and tf.io.gfile.exists(metadata_path):
            ...
            while n_examples % batch_size != 0:
                writer.write(
                    self._make_tf_example(
                        task_id=len(self._config.task_names)
                    ).SerializeToString()
                )
                n_examples += 1
        return n_examples
