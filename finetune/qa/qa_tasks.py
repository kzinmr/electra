...
class QATask(task.Task):
    ...
    def __init__(
        self, config: configure_finetuning.FinetuningConfig, name, tokenizer, v2=False
    ):
    ...
    def get_prediction_module(self, bert_model, features, is_training, percent_done):
        ...
        end_top_index = tf.zeros(
            [batch_size, self.config.beam_size, self.config.beam_size], tf.int32
        )
        if self.config.joint_prediction:
            ...
            if not is_training:
                ...
                end_features = tf.tile(
                    tf.expand_dims(final_hidden, 1), [1, self.config.beam_size, 1, 1]
                )
                # batch, beam, length
                ...
                start_features = tf.reduce_sum(
                    tf.expand_dims(final_hidden, 1) * tf.expand_dims(start_index, -1),
                    axis=-2,
                )
                # batch, beam, length, hidden
                start_features = tf.tile(
                    tf.expand_dims(start_features, 2), [1, 1, seq_length, 1]
                )
            else:
                start_index = tf.one_hot(
                    features[self.name + "_start_positions"],
                    depth=seq_length,
                    axis=-1,
                    dtype=tf.float32,
                )
        ...
        if self.config.answerable_classifier:
            ...
            if self.config.answerable_uses_start_logits:
                ...
                start_feature = tf.reduce_sum(
                    tf.expand_dims(start_p, -1) * final_hidden, axis=1
                )
...
class MRQATask(QATask):
    ...
    def get_examples(self, split):
        ...
        with tf.io.gfile.GFile(
            os.path.join(self.config.raw_data_dir(self.name), split + ".jsonl"), "r"
        ) as f:
        ...
        utils.log(
            "{:} examples created, {:} failures".format(
