...
from finetune.qa import (
    mrqa_official_eval,
    squad_official_eval,
    squad_official_eval_v1,
)
...
class SpanBasedQAScorer(scorer.Scorer):
    ...
    def _get_results(self):
        ...
        if self._name == "squad":
            ...
            return sorted(
                utils.load_json(self._config.qa_eval_file(self._name)).items()
            )
        elif self._name == "squadv1":
            return sorted(
                squad_official_eval_v1.main(self._config, self._split).items()
            )
        else:
            return sorted(
                mrqa_official_eval.main(self._config, self._split, self._name).items()
            )
    ...
    def write_predictions(self):
        ...
        for example in self._eval_examples:
            ...
            for (feature_index, feature) in enumerate(features):
                ...
                if self._config.joint_prediction:
                    ...
                    start_indexes = _get_best_indexes(
                        result.start_logits, self._config.n_best_size
                    )
                    end_indexes = _get_best_indexes(
                        result.end_logits, self._config.n_best_size
                    )
                ...
                for i, start_index in enumerate(start_indexes):
                    for j, end_index in enumerate(
                        ...
                        if (
                            start_index
                            not in feature[self._name + "_token_to_orig_map"]
                        ):
                            continue
            ...
            for pred in prelim_predictions:
                ...
                orig_doc_end = feature[self._name + "_token_to_orig_map"][
                    pred.end_index
                ]
