...
class SentenceLevelScorer(scorer.Scorer):
    ...
    def update(self, results):
        ...
        self._true_labels.append(
            results["label_ids"] if "label_ids" in results else results["targets"]
        )
...
class MCCScorer(SentenceLevelScorer):
    def _get_results(self):
        return [
            (
                "mcc",
                100 * sklearn.metrics.matthews_corrcoef(self._true_labels, self._preds),
            ),
            ("loss", self.get_loss()),
        ]
