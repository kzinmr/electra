...
class TaggingTask(task.Task):
            ...
            for _, tags in split_sentences:
                if not self._is_token_level:
                    ...
                    tags = tagging_utils.get_tags(
                        span_labels, len(tags), LABEL_ENCODING
                    )
                for tag in tags:
                    tag_counts[tag] += 1
        ...
        if self.name == "ccg":
            ...
            label_mapping = {
                ...
                for i, label in enumerate(
                    sorted(
                        filter(lambda t: t not in infrequent_tags, tag_counts.keys())
                    )
                )
            }
            n = len(label_mapping)
    ...
    def _get_labeled_sentences(self, split):
        ...
        with tf.io.gfile.GFile(
            os.path.join(self.config.raw_data_dir(self.name), split + ".txt"), "r"
        ) as f:
            sentence = []
            for line in f:
    ...
    def get_feature_specs(self):
        return [
            ...
            feature_spec.FeatureSpec(
                self.name + "_labels", [self.config.max_seq_length]
            ),
            feature_spec.FeatureSpec(
                self.name + "_labels_mask",
                [self.config.max_seq_length],
                is_int_feature=False,
            ),
            feature_spec.FeatureSpec(
                self.name + "_labeled_positions", [self.config.max_seq_length]
            ),
        ]
