...
def scatter_update(sequence, updates, positions):
    ...
    not_first_token = tf.concat(
        [tf.zeros((B, 1), tf.int32), tf.ones((B, L - 1), tf.int32)], -1
    )
...
def get_vocab(config: configure_pretraining.PretrainingConfig):
    ...
    if config.vocab_file not in VOCAB_MAPPING:
        vocab = tokenization.FullTokenizer(config.vocab_file, do_lower_case=True).vocab
...
def mask(
    ...
    inputs_ids, _ = scatter_update(
        inputs.input_ids, tf.fill([B, N], vocab["[MASK]"]), replace_with_mask_positions
    )
